...
### What it does to FIELDWORK measurement 7 (was 8)
...
reading measurement 7 now asks for. **One reading settles Po and the grade at the
same time.**
...
## D41 — The thermistor divider default is the 450's: 25.75 k pull-up, hotter reads lower

`thermal.c` shipped `THERMAL_HOTTER_IS_HIGHER 1` with a 20 k pull-down: NTC to
VDD, a hotter head reads HIGHER. D30 had already established the opposite from
the only working reference: the 450 firmware's thermistor channel reads
**higher = colder**, and a single ~25.75 k pull-up with the NTC to ground
reproduces its halt/resume thresholds (176 / 255 on 10 bits) at 70 / 56 C to
about 1 %. The code and its own evidence disagreed, and the rule is to start
from the 450.

Decision: `THERMAL_HOTTER_IS_HIGHER 0`, `R_p` 25.75 k. The three `THERMAL_*_RAW`
codes and the dwell table are regenerated by `tools/gen_thermal_table.py` for
that divider (normalised codes 1891 / 3068 / 3398 at 25 / 56 / 70 C; the
inverted pull-up code equals the pull-down code for the same `R_p`, so the
firmware's normalisation is unchanged). `test_thermal.c` models the pull-up
board's raw pin and pins the 450 correspondence: its own model must give 176
and 255 on 10 bits at 70 and 56 C, so a retune away from the vendor's divider
fails a test.

What it changes on a bring-up board: with the wrong orientation a cold head
reads as hot, `thermal_ok()` is false, every line waits its bounded second and
prints at 0.625x, and the panel blinks the error pattern - a printer that
"does not work" for no visible reason. With the 450's orientation as the
default that failure needs the 550 board to differ from the 450 in a way
nothing suggests. The 550 board is still its own design, so FIELDWORK 3 stays
on the list; it is two ADC readings and a thermometer.

## D42 — The top-of-form photocell is analog, with the 450's Schmitt trigger

FIELDWORK 3.1 row 7: the 450 firmware converts its label-gap sensor on an ADC
channel and squares it up with a software Schmitt trigger at 294 / 320 counts.
`pins.h` had a digital `PIN_PAPER_SENSE` with a pull-up and a GPIO read, and
FIELDWORK 4 asked the fieldworker to find out "digital or analog". The vendor
answered that already.

Decision: `PAPER_SENSE_ANALOG 1` - PA0 as ADC_IN0, thresholds 1176 / 1280 (the
450's scaled to 12 bits), hysteresis exactly as the vendor: the state changes
only past the far threshold. `paper_present()` in `thermal.c` (it owns the ADC)
replaces the three GPIO reads in `main.c` and `protocol.c`; the digital path
stays behind `PAPER_SENSE_ANALOG 0` for a board with a true logic-level sensor.
Assumed, and marked so in `pins.h`: the pad, and that more light reads higher
(`PAPER_ADC_HIGH_IS_ABSENT`). Nothing that prints depends on this -
`OP_FLAG_PAPER_FORCE` is the default - so the assumption costs the panel light
at worst, and `GS D 0x06` shows the channel and its direction in one scan.
`test_thermal.c` checks the hysteresis in both directions.

Also settled by the same table and worth recording: the 450 has **two
debounced digital inputs** that abort a job (rows 18-19; cover switch and
paper-out are the obvious candidates) and a **VH rail divider** it suspends
