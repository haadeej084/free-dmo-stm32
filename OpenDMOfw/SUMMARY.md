...
OpenDMO-FW deliberately **clones** the genuine Dymo LabelWriter 550 / 5XL: real USB
identity (VID `0x0922`, per-model PID), real wire protocol, real roll-state semantics —
so stock DYMO Connect enumerates it unchanged and any physical roll prints. See
...
| B8 | Host sender `tools/opsend.py` (genuine Dymo protocol via libusb, PNG→raster) | OK |

## Label counter (Dymo-like, as requested)

Counts down per **printed label** (not per dot line) and is written persistently to
EEPROM, so the value survives a power cycle — a Dymo-like roll counter **without** any
...
- Identity: cloned genuine Dymo — VID `0x0922`, PID `0x002A` (5XL) / `0x0028` (550),
