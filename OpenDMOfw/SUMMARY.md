...
| C3 | Worst-case stack (968 OP104 / 824 OP57 of 2048 bytes) and static analysis (GCC analyzer, cppcheck) | OK |
...
Narrowed to **ten** measurements — see `FIELDWORK.md` section 8, which opens
...
D35/D36 then added the flex pin count (5b) and the head's Po (8).
...
paper-sensor type, the VH enable pin (5), the flex pin count (5b), the strobe
polarity (6b), the half-2 dot order (6, only if 5b finds two data pins), host
acceptance (7) and Po at the head (8) — ten, matching the count above.
