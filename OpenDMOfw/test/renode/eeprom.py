...
MAGIC = 0x4F444D31                      # store.c CFG_MAGIC "ODM1"
...
def record(sku, count, density=8, flags=FLAG_PAPER_FORCE):
    return struct.pack("<I24sHBB", MAGIC, sku.encode(), count, density, flags)
...
def run(part, preload, writable=True, read_len=0x140):
    ...
    if len(vals) != n + 32 + 2:
        return None, out
    ee = bytes(vals[:n])
    ram = bytes(vals[n:n + 32])
    return dict(ee=ee, ram=ram, addrw=vals[n + 32], in_main=main_lo <= vals[n + 33] < main_hi), out


def unpack(rec):
    magic, sku, count, density, flags = struct.unpack("<I24sHBB", rec)
...
def main():
    ...
    def exp_big_blank(r):
        ...
        if unpack(r["ee"][0x100:0x120])[:3] != (MAGIC, sku0, count0): f.append("defaults not persisted at 0x100")
    ...
    def exp_small_blank(r):
        ...
        if unpack(r["ee"][0:32])[:3] != (MAGIC, sku0, count0): f.append("defaults not persisted at 0")
        if unpack(r["ram"])[:3] != (MAGIC, sku0, count0): f.append("RAM config is not the defaults")
        return f
    ...
    def exp_loaded(r):
        ...
        if r["ee"][0x100:0x120] != custom: f.append("stored record was rewritten")
        return f
    ...
    def exp_wp(r):
        f = []
        if unpack(r["ram"])[:3] != (MAGIC, sku0, count0): f.append("RAM config is not the defaults")
        if any(r["ee"][0x100:0x120]): f.append("write-protected part was written")
        return f
